...
def send_line_alert(action: str, reason: str, price=None):
    if action not in ["BUY", "SELL"]:
        return
    # ลบ HTML tags เพื่อให้ข้อความใน LINE อ่านง่ายขึ้น
    clean_reason = reason.replace("<strong>", "").replace("</strong>", "").replace("<br/>", "\n").replace("<i>", "").replace("</i>", "")
...
cursor = conn.cursor()
cursor.execute("CREATE TABLE IF NOT EXISTS logs (id INTEGER PRIMARY KEY AUTOINCREMENT, action TEXT, price REAL, reason TEXT, timestamp TEXT)")
conn.commit()
...
def get_thai_time(): return datetime.utcnow() + timedelta(hours=7)
...
def get_trading_period(now):
    ...
    if dt_time(0, 0) <= current_time <= dt_time(1, 59, 59):
        ...
        if logical_date.weekday() < 5: return "WD_Late_Night", "Weekday Late Night", True, datetime.combine(current_date, dt_time(1, 59, 59))
    if weekday < 5:
        if dt_time(6, 0) <= current_time <= dt_time(11, 59, 59): return "WD_Morning", "Weekday Morning", True, datetime.combine(current_date, dt_time(11, 59, 59))
        elif dt_time(12, 0) <= current_time <= dt_time(17, 59, 59): return "WD_Afternoon", "Weekday Afternoon", True, datetime.combine(current_date, dt_time(17, 59, 59))
        elif dt_time(18, 0) <= current_time <= dt_time(23, 59, 59): return "WD_Evening", "Weekday Evening", True, datetime.combine(current_date, dt_time(23, 59, 59))
    else:
        if dt_time(9, 30) <= current_time <= dt_time(17, 29, 59): return "WE_Active", "Weekend Active", True, datetime.combine(current_date, dt_time(17, 29, 59))
...
def save_portfolio(portfolio):
    with open(config.PORTFOLIO_FILE, "w") as f: json.dump(portfolio, f, indent=4)
...
def save_chart_history(history):
    with open("chart_history.json", "w") as f: json.dump(history, f)
...
def get_global_markets():
    try:
        ...
        for ticker in ["GC=F", "MGC=F"]:
            try:
                temp_hist = yf.Ticker(ticker).history(period=config.GOLD_HISTORY_PERIOD)['Close']
                if len(temp_hist) >= config.EMA_SLOW: gold_hist = temp_hist; break
            except: continue
        ...
        down = -1 * delta.clip(upper=0).ewm(alpha=1/config.RSI_PERIOD, adjust=False).mean()
        rsi = 100 - (100 / (1 + (up / down))).iloc[-1]
        thb_hist = yf.Ticker("THB=X").history(period=config.FOREX_HISTORY_PERIOD)['Close']
...
def get_news():
    news_items = []
    feeds = [("Bangkok Post", "https://www.bangkokpost.com/rss/data/business.xml"), ("Yahoo Finance", "https://finance.yahoo.com/news/rssindex"), ("MarketWatch", "http://feeds.marketwatch.com/marketwatch/topstories/"), ("WSJ Markets", "https://feeds.a.dj.com/rss/RSSMarketsMain.xml")]
    for source, url in feeds:
        try:
            response = requests.get(url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=8)
            for entry in feedparser.parse(response.content).entries[:2]: news_items.append(f"[{source}] - {entry.get('title')}")
        except: continue
    return "\n".join(news_items) if news_items else "News feed offline."

def ask_groq(prompt):
    models = ["llama-3.1-8b-instant", "llama-3.3-70b-versatile"]
    if not GROQ_API_KEYS: return PROMPTS[LANGUAGE]["error_groq"]
    for api_key in GROQ_API_KEYS:
        try:
            return Groq(api_key=api_key).chat.completions.create(messages=[{"role": "system", "content": PROMPTS[LANGUAGE]["system"]}, {"role": "user", "content": prompt}], model=models[0], temperature=0.1, max_tokens=600).choices[0].message.content
        except: continue
...
def log_to_json(log_entry):
    ...
    with open(config.LOG_FILE_NAME, "w", encoding="utf-8") as f: json.dump(logs, f, indent=4, ensure_ascii=False)
...
def run_ai_analysis_logic():
    ...
    weekend_rule = f" WEEKEND MODE: Use minimum amount (AMOUNT_THB: {config.TRADE_MIN_THB:.2f})." if "WE_" in period_key else ""
    
    if target_trades == 0: dynamic_quota = "No strict quota. Trade on clear convergence." + weekend_rule
    elif current_trades < target_trades and minutes_remaining <= 60: dynamic_quota = f"URGENT: MUST trade NOW to pass limit ({minutes_remaining} mins left). Use AMOUNT_THB: {config.TRADE_MIN_THB:.2f}." + weekend_rule
    else: dynamic_quota = f"Quota Status: {current_trades}/{target_trades}. Trade normally." + weekend_rule

    lstm_live_data = {'HSH_Buy': market['HSH_Buy'], 'HSH_Sell': market['HSH_Sell'], 'xau_price': global_math['xau_price'], 'current_thb': global_math['current_thb'], 'rsi': global_math['rsi']}
    ...
    return {
        ...
        "current_market_price": market["HSH_Sell"] if ai_act == "BUY" else market["HSH_Buy"]
    }
    ...
    while True:
        try:
            ...
            if res and 'Ask965' in res and 'Bid965' in res:
                ...
                history.append({"timestamp": get_thai_time().isoformat(), "price": float(str(res['Ask965']).replace(',', '')), "buy": float(str(res['Bid965']).replace(',', ''))})
...
def get_chart_data(): return {"status": "success", "data": load_chart_history()}

@api_router.get("/news")
def get_latest_news_api():
    try: return {"status": "success", "news": [{"title": e.get('title'), "link": e.get('link')} for e in feedparser.parse("https://www.bangkokpost.com/rss/data/business.xml").entries[:5]]}
    except: return {"status": "error", "news": []}
...
def get_status():
    ...
    if portfolio.get("Current_Date") != str(now.date()) or portfolio.get("Current_Period") != period_key:
        portfolio["Current_Date"] = str(now.date()); portfolio["Current_Period"] = period_key; portfolio["Trades_Count"] = 0
    ...
    report = AdvancedTradingAnalytics.generate_full_report(closed, unrealized, nav, first_date)

    return {
        "portfolio": portfolio, "market": market, "net_asset_value": nav,
        "period": {"name": period_name, "is_active": is_active, "trades_done": portfolio["Trades_Count"]},
        "performance": { "total_closed_trade": report["Total Closed Trade"], "win_rate": report["Win Rate (%)"], "total_profit": report["Total Profit (THB)"], "unrealized_pl": report["Unrealized P/L (THB)"], "avg_win": report["Average Win (THB)"], "avg_loss": report["Average Loss (THB)"], "expectancy": report["Expectancy per Trade (THB)"], "best_trade": report["Best Annualized Trade (%)"], "worst_trade": report["Worst Annualized Trade (%)"], "median_trade": report["Median Annualized Trade (%)"], "top10_trade": report["Top 10% Annualized Trade (%)"], "bottom10_trade": report["Bottom 10% Annualized Trade (%)"], "xirr": report["XIRR (%)"], "avg_capital_year": report["Avg Capital/Year (THB)"], "sharpe_ratio": report["Sharpe Ratio"] }
    }

@api_router.post("/portfolio")
def update_portfolio(req: PortfolioUpdate):
    portfolio = load_portfolio()
    portfolio['THB_Balance'] = req.THB_Balance; portfolio['Gold_Gram'] = req.Gold_Gram
    save_portfolio(portfolio); return {"status": "success"}
...
def trigger_analysis():
    return run_ai_analysis_logic() or {"error": "Market Offline"}
...
def execute_trade(req: ExecuteRequest):
    ...
    log_to_json({"date": now.strftime('%Y-%m-%d %H:%M:%S'), "period": period_key, "action": req.ai_action, "user_action": req.user_action, "executed_action": act, "price": exec_price, "reason": req.ai_reason, "amount": exec_amt_str, "total_asset_value": nav})
...
def proxy_hsh(path: str):
    try: return requests.get(f"https://apicheckpricev3.huasengheng.com/{path}", headers={'User-Agent': 'Mozilla/5.0'}, timeout=5).json()
    except: return {}
